 ...
 * 
 */
public abstract class BaseSwipeOperationActivity extends BaseSwipeBackAcvitiy {
	...
	public static final String JIEBAO_HT380 = "HT380";//小黄机型号
	public static final String UROVO_I6200S = "i6200S";//黑色手机型号
	...
	protected void onResume() {
		...
		} else if(model.equals(UROVO_I6200S) ||model.equals(UROVO_I6200A)) {//||model.equals("i6160S")
			IntentFilter UrovoFilter = new IntentFilter();
			UrovoFilter.addAction(SCAN_ACTION);
			registerReceiver(mUrovoReceiver, UrovoFilter);
		} else if (model.equals(GIONEELY72_KK) || model.equals("Android")){
...
//            scanner.enablShowNoticeIcon(true);//是否允许显示iscan通知栏图标
	...
	protected void onPause() {
		...
		}else if (model.equals(UROVO_I6200S) || model.equals(UROVO_I6200A)) {
	...
	private BroadcastReceiver mReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(ScanService.CODE_RESULT)) {
				String code = intent.getStringExtra(ScanService.CODE).toUpperCase().trim().replace(" ", "");
				LogUtils.logOnFile("-----jiebao scan code:"+code);
				if(StringUtils.isBlank(code)){
					((HotApplication)getApplication()).getsoundPool(4);
					Toast.makeText(context, "条码有问题，请确认", Toast.LENGTH_SHORT).show();
					return;
				}
				fillCode(code);
			}
	...
	private BroadcastReceiver mUrovoReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			LogUtils.logOnFile("-----urovo scan code:"+barcodeStr);
			if(!isScan){
    ...
    private class ScannerResultReceiver extends BroadcastReceiver{
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(RES_ACTION)) {
                ...
                LogUtils.logOnFile("-----idata scan code:"+barocodeStr);
                if(StringUtils.isBlank(barocodeStr)){
                	((HotApplication)getApplication()).getsoundPool(4);
					Toast.makeText(context, "条码有问题，请确认", Toast.LENGTH_SHORT).show();
					return;
				}
