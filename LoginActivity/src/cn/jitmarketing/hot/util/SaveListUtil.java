...
//				mJson.put("SKUCode", sb.SKUCode);
	...
	public static String shelfSave(String receiveSheetNo, List<InStockDetail> list) {
		try {
			...
			for(InStockDetail sb : list) {
				...
				mJson.put("SKUCount", sb.SKUCount);
				mJson.put("ShelfLocationCode", sb.ShelfLocationCode);
				mJson.put("intervalTime", sb.intervalTime);
				array.put(mJson);
			}
			postData.put("SKUList", array);
			return postData.toString();
	 ...
	 * @return
	 */
	public static String CGYStockSave(String ShelfLocationCode, ArrayList<SkuBean> list, Context context) {
		try {
			JSONObject postData = new JSONObject();  
			postData.put("UnitID", HotApplication.getInstance().getUnitId()); 
			postData.put("UserID", HotApplication.getInstance().getUserId()); 
			postData.put("ShelfLocationCode", ShelfLocationCode); 
			postData.put("MACAddress", MacUtils.getMobileMAC(context)); 
			JSONArray array = new JSONArray();
			for(SkuBean sb : list) {
				JSONObject mJson = new JSONObject();  
				mJson.put("SKUCode", sb.skuCode.toUpperCase().trim());
				mJson.put("SKUCount", sb.skuCount);
				mJson.put("ScanTime", sb.scanTime);
				array.put(mJson);
			}
			postData.put("Data", array);
			return postData.toString();
	 ...
	 * @return
	 */
	public static String chouPanConfirm(List<InStockDetail> list){
		JSONObject postData = new JSONObject();  
		try {
			postData.put("userid ", HotApplication.getInstance().getUserId());
			postData.put("unitid ", HotApplication.getInstance().getUnitId());
			...
			for(InStockDetail sb : list) {
				JSONObject mJson = new JSONObject();  
				mJson.put("skuid", sb.SKUCode.toUpperCase().trim());
				mJson.put("qty", sb.SKUCount);
				mJson.put("shelflocationcode", sb.ShelfLocationCode);
				array.put(mJson);
			}
			postData.put("Data", array);
			...
			LogUtils.logOnFile("抽盘提交json解析异常->" + ExceptionUtils.getStackTrace(e));
		} 
		return null;
	}
}
