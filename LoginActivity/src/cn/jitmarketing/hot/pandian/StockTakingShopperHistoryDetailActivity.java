...
import cn.jitmarketing.hot.entity.BigClass;
...
public class StockTakingShopperHistoryDetailActivity extends BaseSwipeOperationActivity implements
	...
	protected void exInitView() {
		...
		searchAdapter = new StockTakingDetailAdapter(this, searchList, true); 
		search_list.setAdapter(searchAdapter);
		initMenu();
	}
	
	private String mAscode = "0";
	private int[] bigTypes = new int[]{1,2,3,4,5,6,7,0};
	private List<BigClass> mBigClasses = new ArrayList<BigClass>();
	/****
     * 菜单
     */
    void initMenu() {
    	String[] mItems = getResources().getStringArray(R.array.bigtypes);
    	mBigClasses.clear();
    	for(int i=0; i<mItems.length; i++){
    		mBigClasses.add(new BigClass(bigTypes[i]+"", mItems[i]));
    	}
		menu_list.setAdapter(new BigClassAdatper());
        menu_list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ll_menu.setVisibility(View.GONE);
                detail_title.setRightText(mBigClasses.get(position).sname);
                mAscode = mBigClasses.get(position).scode;
                queryLocation();
            }
        });
    }
    
    private class BigClassAdatper extends BaseAdapter {

        @Override
        public int getCount() {
            return mBigClasses.size();
        }

        @Override
        public Object getItem(int position) {
            return mBigClasses.get(position);
        }
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            textView.setText(mBigClasses.get(position).sname);
	...
	public Map<String, String> onStart(int what) {
		...
		case WHAT_NET_SHELF_LIST:
			return WarehouseNet.getShelfHistoryDetail(ShelfLocationCode, CheckTaskID, String.valueOf(pageIndex), String.valueOf(pageSize), mAscode);
	...
	public void onLoad() {
		...
		Ex.Net(mContext).sendAsyncPost(
				...
				WarehouseNet.getShelfHistoryDetail(ShelfLocationCode, CheckTaskID, String.valueOf(pageIndex), String.valueOf(pageSize), mAscode), requestCallback);			
	}
	...
	public void onRefresh() {
		...
		Ex.Net(mContext).sendAsyncPost(
				HotConstants.Global.APP_URL_USER + HotConstants.Shelf.LoadShelfHistorylocation,
				WarehouseNet.getShelfHistoryDetail(ShelfLocationCode, CheckTaskID, "0", String.valueOf(pageSize), mAscode), requestCallback);		
