...
public class LoginActivity extends BaseOperationActivity {  
	...
	private void login() {
		...
		MgrPerference.getInstance(this).putString("CACHE_APP_URL", HotConstants.Global.APP_URL_USER);
	}
	...
	public void onSuccess(int what, String result,boolean isCache) {
		...
		case WHAT_NET_LOGIN:
			try {
				...
				LogUtils.logOnFile("当前APP版本："+getPackageManager().getPackageInfo(getPackageName(), 0).versionName);
			} catch (Exception e) {
				LogUtils.logOnFile("登录异常："+ExceptionUtils.getStackTrace(e));
				Toast.makeText(mActivity, "登录异常", Toast.LENGTH_LONG).show();
			}
