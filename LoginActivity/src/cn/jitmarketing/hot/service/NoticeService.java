...
public class NoticeService extends Service {
	...
	private com.rabbitmq.client.Connection conCustomer = null;
	private com.rabbitmq.client.Channel channelCustomer = null;
	...
	public void onDestroy() {
		...
		if (channelCustomer != null) {
			try {
				...
				Log.d("Service", "顾客mq通道关闭失败!" + e.getMessage());
			} catch (IOException e) {
				Log.d("Service", "顾客mq通道关闭失败!" + e.getMessage());
			}
		...
		if (conCustomer != null) {
			try {
				...
				Log.d("Service", "顾客mq连接关闭失败!" + e.getMessage());
			}
			conCustomer = null;
		}
	...
	private void colse() {
		...
		if (channelCustomer != null && channelCustomer.isOpen()) {
			try {
				channelCustomer.close();
			} catch (TimeoutException e) {
				Log.d("Service", "顾客mq通道关闭失败!" + e.getMessage());
	...
	protected void doLoopCustomerMQMessage() {
		LogUtils.logOnFile("-----Customer doLoopCustomerMQMessage start-----");
		String role = MgrPerference.getInstance(this).getString(HotConstants.Unit.ROLE_CODE);
		ArrayList<GroupEntity> groupList = (ArrayList<GroupEntity>) MgrPerference.getInstance(this).getObject(role);
		boolean isok = false;
		while (!isok) {
			try {
				...
				cfa.setUsername("swms");
				cfa.setPassword("hotwind@2015");
				cfa.setHost("10.0.19.130");
				conCustomer = cfa.newConnection();
				channelCustomer = conCustomer.createChannel();
				channelCustomer.exchangeDeclare("swms", "topic", true);
				String queuename = channelCustomer.queueDeclare().getQueue();// MgrPerference.getInstance(this).getString(HotConstants.User.USER_CODE)+
																		// new
																		// Date().getTime();

				Consumer consumer = new DefaultConsumer(channelCustomer) {
					@Override
					public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
						String message = new String(body, "UTF-8");
						Log.w("Customer NOTICE", " [x] Received '" + envelope.getRoutingKey() + "':'" + message + "'");
						LogUtils.logOnFile("Customer推送消息：" + message);
						handler.sendMessage(handler.obtainMessage(1, message));
					}
				};
				for (GroupEntity group : groupList) {
					for (NoticeSettingEntity noticeEntity : group.getSublist()) {
						if (noticeEntity.getOpen()) {
							String routekey = noticeEntity.getKey();
							channelCustomer.queueBind(queuename, "swms", routekey);
						}
					}
				}
				channelCustomer.basicConsume(queuename, false, consumer);
				isok = true;
				LogUtils.logOnFile("Customer RabbitMQ连接成功");
			} catch (Exception ex) {
				ex.printStackTrace();
			}
			try {
