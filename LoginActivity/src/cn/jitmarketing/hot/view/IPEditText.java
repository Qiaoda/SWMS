...
public class IPEditText extends LinearLayout {
	...
	public IPEditText(Context context, AttributeSet attrs) {
		...
		View view = LayoutInflater.from(context).inflate(
				R.layout.custom_edittext_ip, this);
...
//						editor.commit();
				...
				if (start == 0 && s.length() == 0) {
					...
					// mFirstIP.setSelection(mPreferences.getInt("IP_FIRST",
					// 0));
				}
...
//						editor.commit();
				...
				if (start == 0 && s.length() == 0) {
					mSecondIP.setFocusable(true);
					mSecondIP.requestFocus();
					// mSecondIP.setSelection(mPreferences.getInt("IP_SECOND",
					// 0));
				}
			}
		...
		mFourthIP.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before,
				...
				if (s != null && s.length() > 0) {
					...
					if (!mText4.equals("") && Integer.parseInt(mText4) > 255) {
...
//					editor.commit();
				...
				if (start == 0 && s.length() == 0) {
					mThirdIP.setFocusable(true);
					mThirdIP.requestFocus();
					// mThirdIP.setSelection(mPreferences.getInt("IP_THIRD",
					// 0));
				}
			}
			@Override
	...
	public boolean invalid() {
		...
		if (mText1.equals("") || mText2.equals("") || mText3.equals("")
				|| mText4.equals("")) {
			return false;
		}
		if (Integer.parseInt(mText1) > 255 || Integer.parseInt(mText2) > 255
				|| Integer.parseInt(mText3) > 255
