...
import cn.jitmarketing.hot.SearchInStockDiffActivity;
 ...
 * 入库差异
 */
public class InStockDiffActivity extends BaseSwipeOperationActivity implements OnClickListener, OnItemClickListener {
	...
	protected int exInitLayout() {
		return R.layout.activity_instock_diff;
	}
	@Override
	...
	protected void exInitView() {
		...
		initListViewFooter();
		and_list.setAdapter(inStockDiffListAdapter);
		refreshableView.setOnRefreshListener(new PullToRefreshListener() {
			@Override
			public void onRefresh() {
				// 日志操作
				LogUtils.logOnFile("入库差异->刷新入库列表");
				PageIndex = 1;
				stateLoaded();
				Ex.Net(mContext).sendAsyncPost(HotConstants.Global.APP_URL_USER + HotConstants.Global.IN_STOCK_DIFF, WarehouseNet.getInStockList(PageIndex, PageSize), requestCallback);
			}
		}, 0);
		startTask(HotConstants.Global.APP_URL_USER + HotConstants.Global.IN_STOCK_DIFF, WHAT_NET_GET_IN_STOCK_LIST, NET_METHOD_POST, false);
				...
				case OnScrollListener.SCROLL_STATE_IDLE:// 当不滚动时
					if (view.getLastVisiblePosition() == view.getCount() - 1 && TotalCount != 0) {// 已滚动到最底部
						if (receList.size() < TotalCount) {
							...
							startTask(HotConstants.Global.APP_URL_USER + HotConstants.Global.IN_STOCK_DIFF, WHAT_NET_GET_IN_STOCK_LIST, false, NET_METHOD_POST, false);
	...
	public void onError(int what, int e, String message) {
		...
		case WHAT_NET_GET_IN_STOCK_LIST:
			...
			// 日志操作
			LogUtils.logOnFile("入库差异->" + getText(R.string.getInfo).toString());
			break;
		}
	}
	...
	public void onSuccess(int what, String result, boolean hashCache) {
		...
		case WHAT_NET_GET_IN_STOCK_LIST:
			stockComplete = true;
			// 日志操作
			LogUtils.logOnFile("入库差异->获取列表数据");
			try {
				JSONObject jsonObject = new JSONObject(mGson.toJson(net.data));
				String stockStr = jsonObject.getString("List");
	...
	public void onClick(View v) {
		...
		case R.id.lv_left:
			...
			break;
		case R.id.lv_right:
			startActivity(new Intent(mActivity, SearchInStockDiffActivity.class));
			break;
		}
	}
		...
		if (canReceive) {
			...
			for (int i = 0; i < receList.size(); i++) {
				if (code.equals(receList.get(i).DisOrderNo.toUpperCase())) {
					and_list.setSelection(i);
					inStockDiffListAdapter.setSelectItem(i);
					inStockDiffListAdapter.notifyDataSetInvalidated();
					isHave = false;
					return;
				}
			}
			if (isHave) {
				Intent intent = new Intent(InStockDiffActivity.this, HistoryInStockActivity.class);
