...
public class CreatStockTaskActivity extends BaseSwipeBackAcvitiy implements OnClickListener {
	...
	private List<String> noticenNames = new ArrayList<>();
	...
	protected void exInitView() {
		LogUtils.logOnFile("盘点任务->");
		create_spinner = (Spinner) findViewById(R.id.create_spinner);
		adapter = new ArrayAdapter(this, android.R.layout.simple_spinner_item, noticenNames);
	...
	public Map<String, String> onStart(int what) {
		LogUtils.logOnFile("盘点任务->请求"+what);
		switch (what) {
	...
	public void onError(int what, int e, String message) {
		// 操作日志
		LogUtils.logOnFile("任务盘点->请求失败" + message);
		switch (what) {
		case WHAT_NET_CREATE_OK:
	...
	public void onSuccess(int what, String result, boolean hashCache) {
		// 操作日志
		LogUtils.logOnFile("任务盘点->请求成功" + result);
		switch (what) {
		case WHAT_NET_CREATE_OK:
			...
			try {
				...
				if (infoList.size() > 0) {
					...
					for (int i = 0; i < infoList.size(); i++) {
						...
						noticenNames.add(infoList.get(i).NoticenName);
					}
