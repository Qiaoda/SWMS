...
public class TakeGoodsActivity extends BaseSwipeOperationActivity implements OnClickListener, OnItemClickListener {
	...
	private List<TakeGoodsEntity> supplementLists = new ArrayList<TakeGoodsEntity>();
	...
	private List<TakeGoodsEntity> search_supplement = new ArrayList<TakeGoodsEntity>();
	private int tab = 1;
	...
	private TakeGoodsListAdapter takeGoodsListAdapterSupplement;// 补量
		...
		if (isRefreshList) {
			if (selectEntity != null) {
				if (tab == 1) {
					...
					supplementLists.remove(selectEntity);
					takeGoodsListAdapterSupplement.notifyDataSetChanged();
				}
			}
			outedSKUCount++;
			notOutSKUCount--;
		...
		if (bd != null) {// 点击通知，查看是哪种类型的出货
			...
			if (tabString != null) {
				if (tabString.equals("quxin")) {
					...
					tvSupplement.setTextColor(getResources().getColor(R.color.color_black_text));
					only_list.setAdapter(takeGoodsListAdapter1);
				} else if(tabString.equals("chuyang")){
					tab = 2;
				...
				}else if(tabString.equals("supplement")){
					tab = 3;
					tvSupplement.setBackgroundResource(R.drawable.tab_center_true);
					tvSupplement.setTextColor(getResources().getColor(R.color.color_white_text));
					text1.setTextColor(getResources().getColor(R.color.color_black_text));
					text2.setTextColor(getResources().getColor(R.color.color_black_text));
					only_list.setAdapter(takeGoodsListAdapterSupplement); 
				}
			} else {
				tab = 1;
				text1.setBackgroundResource(R.drawable.tab_left_true);
	...
	protected void exInitView() {
		...
		tvSupplement.setOnClickListener(this);
		text1.setTextColor(getResources().getColor(R.color.color_white_text));
		text2.setTextColor(getResources().getColor(R.color.color_black_text));
		tvSupplement.setTextColor(getResources().getColor(R.color.color_black_text));
		...
		takeGoodsListAdapterSupplement = new TakeGoodsListAdapter(this, supplementLists);
		only_list.setAdapter(takeGoodsListAdapter1);
		processExtraData();
		if (role != null && role.equals("DZ"))
	 ...
	 * @param shelfCode
	 */
	private void searchSupplementValue(String skuCode, String shelfCode) {
		for (TakeGoodsEntity takeGooge : supplementLists) {
			if (skuCode.equals(takeGooge.SKUID) && (takeGooge.Qty.equals("1") || takeGooge.Qty.equals("0.5") || takeGooge.Qty.equals("0"))) {
				boolean isExist = false;
				for (int i = 0; i < takeGooge.CanOperShelfLocationCodes.length; i++) {
					if (shelfCode.equals(takeGooge.CanOperShelfLocationCodes[i])) {
			...
			if (!isScanSku) {
				...
				if (!SkuUtil.isWarehouse(barcode)) {// 扫描sku码结束，如果接着扫描sku码
					...
					if (tab == 1) {
						...
						searchSupplementValue(skuCode, shelfCode);
					}
					isScanSku = false;// 重置
				}
			}
	...
	public void onSuccess(int what, String result, boolean hashCache) {
		...
		case WHAT_NET_GET_STOCK_LIST:
			...
			try {
				...
				quNew.clear();
				chuyang.clear();
				supplementLists.clear();
				for (int i = 0; i < stockList.size(); i++) {
					if (stockList.get(i).OpID.equals("1024") || stockList.get(i).OpID.equals("2048")) {
						quNew.add(stockList.get(i));
					} else if(StringUtils.isNoneBlank(stockList.get(i).BillNo)){
						supplementLists.add(stockList.get(i));
					}else{
				...
				search_supplement.clear();
				search_supplement.addAll(supplementLists);
				if (tab == 1) {
					takeGoodsListAdapter1.notifyDataSetChanged();
				} else if (tab == 2) {
					takeGoodsListAdapter2.notifyDataSetChanged();
		...
		case WHAT_NET_GET_STOCK:
			...
			if (selectEntity != null) {
				if (tab == 1) {
					...
					supplementLists.remove(selectEntity);
					search_supplement.remove(selectEntity);
					takeGoodsListAdapterSupplement.notifyDataSetChanged();
				}
			}
			outedSKUCount++;
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		if (tab == 1) {
			...
			selectEntity = supplementLists.get(position);
			if (supplementLists.get(position).Qty.equals("1") || supplementLists.get(position).Qty.equals("0.5") || supplementLists.get(position).Qty.equals("0")) {
				skuCode = supplementLists.get(position).SKUID;
				factId = supplementLists.get(position).FactID;
				shelfDialog = new TakeGoodsDialog(mActivity, R.layout.show_shelf_dialog, R.style.Theme_dialog);
				shelfDialog.show();
				shelfText = (SkuEditText) shelfDialog.findViewById(R.id.the_customer_shelf);
				TextView take_goods_ok = (TextView) shelfDialog.findViewById(R.id.take_goods_ok);
				...
				Intent intent = new Intent(this, TakeGoodsBatchActivity.class);
				Bundle bd = new Bundle();
				bd.putString("skuCode", supplementLists.get(position).SKUID);
				bd.putString("factId", supplementLists.get(position).FactID);
				/*if (selectEntity.OpID.equals(HotConstants.SKU.OPT_ID_HY))
					bd.putBoolean("isChangeSample", true);*/
				bd.putInt("count", Integer.valueOf(supplementLists.get(position).Qty));
				intent.putExtras(bd);
				startActivity(intent);
			}
		}
	...
	Runnable runnable = new Runnable() {
		...
		public void run() {
			...
			try {
				...
				if (takeGoodsListAdapterSupplement != null) {
					if (supplementLists != null && supplementLists.size() > 0) {
						for (TakeGoodsEntity en : supplementLists) {
							en.TimeOut = String.valueOf(Long.valueOf(en.TimeOut) + 60);
						}
						takeGoodsListAdapterSupplement.notifyDataSetChanged();
					}
	...
	void resetBtn() {
		...
		tvSupplement.setBackgroundResource(R.drawable.tab_center);
		...
		public void onTextChanged(CharSequence s, int start, int before, int count) {
			if (!s.equals("")) {
				...
				if (tab == 1) {// 取新
					...
					supplementLists.clear();
					for (int i = 0; i < search_supplement.size(); i++) {
						if (search_supplement.get(i).SKUID.contains(s.toString().toUpperCase())) {
							supplementLists.add(search_supplement.get(i));
						}
					}
				}
			} else {
				if (tab == 1) {// 取新
					...
					supplementLists.clear();
					supplementLists.addAll(search_supplement);
					takeGoodsListAdapterSupplement.notifyDataSetChanged();
				}
			}
		}
	...
	private void selectedItem() {
		if (tab == 1) {// 取新
			...
			for (int i = 0; i < supplementLists.size(); i++) {
				if (skuCode.equals(supplementLists.get(i).SKUID)) {
					takeGoodsListAdapterSupplement.setSelectItem(i);
					only_list.smoothScrollToPosition(i);
				}
			}
		}
